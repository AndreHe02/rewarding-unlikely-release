...
class DataProto:
    ...
    def shuffle(self, seed=None):
        """
        Shuffle the data in the DataProto.
        This operation is in-place and uses reorder internally.
        
        Args:
            seed (int, optional): Random seed for reproducibility. Default is None.
            
        Returns:
            DataProto: self, for method chaining
        """
        if self.batch is None and not self.non_tensor_batch:
            return self
            
        batch_size = len(next(iter(self.non_tensor_batch.values()))) if self.non_tensor_batch else self.batch.batch_size[0]
    ...
    def select_indices(self, indices: Union[torch.Tensor, List[int], np.ndarray]) -> 'DataProto':
        """Select rows from the DataProto based on provided indices.
        
        Args:
            indices (Union[torch.Tensor, List[int], np.ndarray]): Indices of rows to select.
                If torch.Tensor, it should contain integer indices.
                If List[int], it will be converted to a tensor.
                If np.ndarray, it will be used directly for non_tensor_batch.
                
        Returns:
            DataProto: A new DataProto containing only the selected rows.
            If indices is empty, returns an empty DataProto with the same structure.
        """
        # Convert indices to appropriate types if needed
        if isinstance(indices, list):
            ...
            tensor_indices = torch.from_numpy(indices).to(
                device=self.batch.device if self.batch is not None else "cpu"
            )
            numpy_indices = indices
        else:
            raise TypeError(f"Indices must be torch.Tensor, List[int], or np.ndarray, but got {type(indices)}")
        
        # Handle empty indices case
        if len(indices) == 0:
            # Create empty tensor batch with the same structure
            if self.batch is not None:
                # Create empty tensors with the same structure but 0 batch dimension
                empty_tensors = {}
                for key, tensor in self.batch.items():
                    # Preserve the tensor shape except for the first dimension (batch)
                    empty_shape = (0,) + tensor.shape[1:]
                    empty_tensors[key] = torch.empty(empty_shape, dtype=tensor.dtype, device=tensor.device)
                
                selected_batch = TensorDict(
                    source=empty_tensors,
                    batch_size=(0,)
                )
            else:
                selected_batch = None
            
            # Create empty non_tensor_batch with the same structure
            selected_non_tensor_batch = {}
            for key, val in self.non_tensor_batch.items():
                # Create empty numpy array with same structure but 0 first dimension
                empty_shape = (0,) + val.shape[1:]
                selected_non_tensor_batch[key] = np.empty(empty_shape, dtype=val.dtype)
        ...
        # Normal case: non-empty indices
        if self.batch is not None:
            selected_batch = self.batch[tensor_indices]
        else:
            selected_batch = None
        
        selected_non_tensor_batch = {}
        for key, val in self.non_tensor_batch.items():
            selected_non_tensor_batch[key] = val[numpy_indices]
