...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str, checkpoint_path=None):
        super().__init__()
        self.config = config
    ...
    def init_model(self):
        ...
        ckpt_or_model_path = self.checkpoint_path if self.checkpoint_path and os.path.exists(self.checkpoint_path) else self.config.model.path
        if ckpt_or_model_path != self.config.model.path:
            print(f"Loading model from checkpoint: {ckpt_or_model_path}")
        ...
        if self._is_ref:
            # always load this from base path
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy", logger=None) as timer:
                if self.config.actor.update_rule == "ppo":
                    metrics = self.actor.update_policy(data=data)
                elif self.config.actor.update_rule == "sft":
                    metrics = self.actor.update_policy_sft(data=data)
                else:
                    raise ValueError(f"Unknown update rule: {self.config.actor.update_rule}")
    ...
    def prepare_for_generation(self):
        """
        Prepares for multiple consecutive generation calls by manually entering
        the rollout sharding manager context.
        """
        self._generation_context_active = True
        self.rollout_sharding_manager.__enter__()
        log_gpu_memory_usage("Entered generation context", logger=logger)


    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_sequences_fast(self, prompts: DataProto):
        """
        Generates sequences without incurring the overhead of repeatedly entering/exiting
        the sharding manager context. Must be called after prepare_for_generation() and before end_generation().
        """
        if not getattr(self, "_generation_context_active", False):
            raise RuntimeError("Call prepare_for_generation() before generate_sequences_fast().")

        assert not self._is_offload_param, "Not handled"
        ...
        # Use the already active sharding manager context.
        prompts = self.rollout_sharding_manager.preprocess_data(prompts)
        output = self.rollout.generate_sequences(prompts=prompts)
        output = self.rollout_sharding_manager.postprocess_data(output)
        output = output.to("cpu")
        ...
        torch.cuda.empty_cache()
        log_gpu_memory_usage("After generate_sequences_fast", logger=logger)
        return output

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def end_generation(self):
        """
        Ends the generation context by exiting the sharding manager context.
        """
        if getattr(self, "_generation_context_active", False):
            # Pass None for exception info since we're not handling an exception here.
            self.rollout_sharding_manager.__exit__(None, None, None)
            self._generation_context_active = False
            log_gpu_memory_usage("Exited generation context", logger=logger)
        else:
            raise RuntimeError("Generation context was not active.")
