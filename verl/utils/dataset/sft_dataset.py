...
class SFTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if not isinstance(prompt, str):
            print(f"[dataset] bad prompt type={type(prompt)}  row={item}  rank={torch.distributed.get_rank()}")
            # optional: raise here to stop immediately
        if not isinstance(response, str):
            print(f"[dataset] bad response type={type(response)}  row={item}  rank={torch.distributed.get_rank()}")


        # if not isinstance(prompt, str):
        #     print("BAD SAMPLE", type(prompt), prompt[:5]) 
