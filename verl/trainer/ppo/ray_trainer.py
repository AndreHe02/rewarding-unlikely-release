...
class RayPPOTrainer(object):
    ...
    def init_workers(self):
        """Init resource pool and worker group, with optional resume from checkpoint"""
        ...
        if getattr(self.config.trainer, "resume", False):
            actor_checkpoint_path, resumed_step, _ = self._find_latest_checkpoint()
            if resumed_step > 0:
        ...
        if self.config.trainer.get("override_resume_checkpoint", None) is not None:
            actor_checkpoint_path = self.config.trainer.override_resume_checkpoint
        ...
        if self.hybrid_engine:
            ...
            actor_rollout_cls = RayClassWithInitArgs(
                ...
                role="actor_rollout",
                checkpoint_path=actor_checkpoint_path  # Pass checkpoint path if resuming
            )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    # this uid is associated with the prompt,
                    # use it to identify the group in GRPO
